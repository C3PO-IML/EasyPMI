...
def format_relative_time(time: float) -> str:
    """
...
def calculate_absolute_dt(pmi_hours: Optional[float], ref_dt: Optional[datetime]) -> Optional[datetime]:
    """
    ...
    """
    if ref_dt is None or pmi_hours is None or math.isnan(pmi_hours) or math.isinf(pmi_hours):
        return None
    try:
        return ref_dt - timedelta(hours=pmi_hours)
...
def format_pmi_range_string(
    ...
    Args:
        ...
        ref_dt: The reference datetime for absolute calculations. If None, uses relative hours.
    ...
    if ref_dt is None:
        ...
        if not min_is_nan and not max_is_nan and not math.isinf(pmi_max_hours) and not math.isclose(pmi_min_hours, 0.0):
            ...
            if pmi_center_hours is not None: # For Henssge/Baccino like results
                 return f" {center_rel} [{min_rel} - {max_rel}]"
            else: # For interval signs
                 return f" between {min_rel} and {max_rel}"
            ...
            return f": Interval undefined ({min_rel} - {max_rel})"
        
    else: # Absolute Time Calculation
        dt_latest = calculate_absolute_dt(pmi_min_hours, ref_dt)
        dt_earliest = calculate_absolute_dt(pmi_max_hours, ref_dt)
        dt_center = calculate_absolute_dt(pmi_center_hours, ref_dt) if pmi_center_hours is not None else None
    ...
    # Build the output string
        if has_earliest and has_latest:
            ...
            if has_center:
                return f" {fmt_center} [{fmt_earliest} | {fmt_latest}]"
            else: # For interval signs
                return f" {range_str}"
        elif has_earliest and not has_latest: # Only earliest makes sense (PMI < max_hours => After earliest)
            return f": After {fmt_earliest}"
        elif not has_earliest and has_latest: # Only latest makes sense (PMI > min_hours => Before latest)
            return f": Before {fmt_latest}"
        else:
            return f": Cannot determine absolute time"

# --- Function for Plot ---

def format_plot_scatter_label(pmi_hours: Optional[float], ref_dt: Optional[datetime]) -> str:
    ...
    if ref_dt is None:
        ...
        dt = calculate_absolute_dt(pmi_hours, ref_dt)
        return f"{format_absolute_datetime(dt)}"

def format_plot_ci_label(pmi_min_hours: Optional[float], pmi_max_hours: Optional[float], ref_dt: Optional[datetime]) -> str:
...
def format_plot_xlabel(ref_dt: Optional[datetime]) -> str:
...
def generate_plot_x_tick_labels(tick_hours: list[float], ref_dt: Optional[datetime]) -> list[str]:
    """
    ...
    if ref_dt is None:
        ...
        for hour in tick_hours:
            ...
            dt = calculate_absolute_dt(hour, ref_dt)
            if dt is None:
...
def format_plot_mustache_labels(pmi_min: Optional[float], pmi_max: Optional[float], pmi_center: Optional[float], ref_dt: Optional[datetime]) -> Tuple[str, str, str]:
    """
    ...
    if ref_dt is None:
        ...
        # Format absolu
        dt_center = calculate_absolute_dt(pmi_center, ref_dt) if has_center else None
        dt_latest = calculate_absolute_dt(pmi_min, ref_dt) if has_min else None    # Min hours -> latest death
        dt_earliest = calculate_absolute_dt(pmi_max, ref_dt) if has_max else None # Max hours -> earliest death
        hour_format = "%Hh%M"
...
def format_plot_zone_label(pmi_limit: Optional[float], side: str, ref_dt: Optional[datetime]) -> Tuple[str, str]:
    """
    ...
    if ref_dt is None:
        ...
        # Format absolu
        dt_limit = calculate_absolute_dt(pmi_limit, ref_dt)
